...
const FRONTEND_BUILD = "2026.08.24-ROUTES-05-CERTIFICATE-01";
...
function renderCertificate() {
  ...
  const character = state.characters.find(item => Number(item.id) === Number(state.selectedCharacter));
  const characterImage = REACTION_IMAGES[state.selectedCharacter]?.happy || character?.image_url || "";
  const characterName = character?.name || "personagem escolhido";
  return `<section class="screen certificate-screen"><article class="certificate certificate-modern" id="certificate-document"><div class="certificate-art" aria-hidden="true"><div class="certificate-wordmark"><span>CERTIFICADO</span></div><div class="certificate-art-line"></div><div class="certificate-art-symbol">π</div></div><div class="certificate-copy"><p class="certificate-congratulations"><u>Parabéns,</u> <strong>${escapeHtml(certificate.player_name)}</strong></p><div class="certificate-body"><p>concluiu com êxito o <strong><u>Desafio Trigonométrico</u></strong>, demonstrando dedicação, raciocínio lógico e conhecimento dos conceitos de trigonometria trabalhados ao longo do desafio.</p><p>O participante obteve <strong>${escapeHtml(certificate.score)} pontos</strong>, alcançando a classificação <strong>${escapeHtml(certificate.title)}</strong> e concluindo sua participação em <strong>${escapeHtml(date)}</strong>.</p><p>Este certificado é concedido como reconhecimento pelo desempenho e pela conclusão do desafio.</p></div><div class="certificate-validation"><span></span><strong>DESAFIO TRIGONOMÉTRICO</strong><small>Certificado de conclusão</small></div>${characterImage ? `<img class="certificate-character" src="${escapeHtml(characterImage)}" alt="${escapeHtml(characterName)} celebrando a conclusão">` : ""}</div></article><div class="certificate-actions"><button class="secondary-button" data-action="back-completed">← VOLTAR</button><button class="secondary-button certificate-canva-button" data-action="open-certificate-canva">ABRIR MODELO NO CANVA</button><button class="primary-button" data-action="print-certificate">IMPRIMIR OU SALVAR EM PDF</button></div></section>`;
}
